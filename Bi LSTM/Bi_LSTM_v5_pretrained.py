...
def load_word_vectors(filename):
    return KeyedVectors.load_word2vec_format(filename, binary=True)  # Use binary=True if loading a binary model like GoogleNews
...
def build_model(input_shape, output_size=20):
    ...
    model.add(Flatten())  # Flattening l-dimensional array into 2D
...
def main():
    if len(sys.argv) != 3:
        sys.exit("Usage: python3 Bi_LSTM_v5_pretrained.py dataset.csv word_vectors.bin")
